...
export function finalAbilityScores(character: Pick<CharacterData, "abilityScores" | "race" | "subrace" | "racialBonusAllocation" | "feats">): AbilityScores {
  ...
  for (const ability of abilities) {
    ...
    for (const feat of character.feats) {
      result[ability] += feat.benefits?.abilityScoreIncrease?.[ability] ?? 0;
    }
  }
  return result;
}
